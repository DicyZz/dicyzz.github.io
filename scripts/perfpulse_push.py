...
1. **LLM 系统 & AI Infra**：
   ...
   - **自定义 News 收藏源**：Understanding AI (understandingai.org), TechCrunch AI (techcrunch.com/category/artificial-intelligence/), Ars Technica (arstechnica.com).

2. **体系结构 & 芯片/IP 微架构**：
   - Chips and Cheese, ServeTheHome, RISC-V International, ACM SIGARCH, IEEE Micro.
   - **自定义 News 收藏源**：SemiEngineering (semiengineering.com), Hardware Times (hardwaretimes.com), WikiChip ARM (wikichip.org), AnandTech (anandtech.com), Design & Reuse (design-reuse.com), EET China 电子工程专辑 (eet-china.com), Doulos (doulos.com), Tom's Hardware (tomshardware.com), ASCII.jp 硬件连载 (ascii.jp), ConsortiumInfo (consortiuminfo.org).
...
4. **Linux 内核 & 系统性能调优**：
   ...
   - **自定义 News 收藏源**：Phoronix (phoronix.com), It's FOSS News (news.itsfoss.com), Slashdot (slashdot.org), Alltop Linux (alltop.com/linux), Narkive 邮件列表 (narkive.com), DZone (dzone.com), Packet Storm Security (packetstormsecurity.com).
...
### 💡 代码与干货要求：
1. **代码与命令包裹**：所有 `perf` 诊断脚本、vLLM 参数、LLVM 编译 Flag 和代码片段，必须明确包裹在 Markdown 代码块中（如 ```bash ... ``` 或 ```python ... ```）。
2. **拒绝陈旧科普**：直奔主题，输出具体参数、指令集、Patch 号、性能提升百分比数据。
...
def send_email(subject, md_content):
    ...
    print("3. 正在使用 Premailer 自动将 CSS 样式转换为内联属性（包含多媒体卡片排版）...")
    ...
    try:
        ...
        server.quit()
        print("🎉 包含全量 News 书签数据源且针对微信公众号排版极致优化的 PerfPulse 简报已成功发送！")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    send_email("【PerfPulse】每日硬件、系统与 LL
